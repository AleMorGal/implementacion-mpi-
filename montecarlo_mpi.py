...
#Funcion que reparte 5 cartas a cada jugador
def repartir_cartas(baraja, numJugadores):
    ...
    jugador = 0
    #Arreglo aque guarda las manos de cada jugador
    manos = []
...
def royal_flush(mano):
    ...
    #Revisa que sean del mismo tipo
    contador_palos = dict(collections.Counter(palos))
    ...
    if mismo_palo == True and corrida_mayor == True:
        resultado_de_mano = 'Flor 1mperlal'
        return resultado_de_mano
    
    return None

#Simulacion del juego
def juego(baraja, numero_de_jugadores) :
    baraja = crear_baraja()
    # Reparte 5 cartas a cada jugador
    manos = repartir_cartas(baraja, numero_de_jugadores)
...
def simulacion_montecarlo(numero_de_jugadores, numero_de_simulaciones):
    baraja = crear_baraja()
    simulacion_actual = 0
    ...
    while numero_de_simulaciones > simulacion_actual:
        ...
        resultados = juego(baraja, numero_de_jugadores)
        
        if resultados[0] == 'Un par':
            un_par += 1
        if resultados[0] == 'Dos pares':
            dos_pares += 1
        if resultados[0] == 'Tercia':
            tercia += 1
        if resultados[0] == 'Color':
            color += 1
        if resultados[0] == 'EScalera':
            escalera += 1
        if resultados[0] == 'Full house':
            full_house += 1
        if resultados[0] == 'Póker':
            poker += 1
        if resultados[0] == 'Escalera de color':
            escalera_de_color += 1
        if resultados[0] == 'Flor imperial':
            flor_imperial + 1
        simulacion_actual += 1
...
if __name__ == "__main__":
    ...
    # Los demas procesos realizan la simulacion y envian sus resultados a el proceso 0
        ...
        resultados = simulacion_montecarlo(4,simulaciones)
    ...
    if rank == 0:
        ...
        cantidad_simulaciones = simulaciones * (size-1)
        ...
        print('\nTotal de combinaciones obtenidas : {}\n'.format(combinaciones))
